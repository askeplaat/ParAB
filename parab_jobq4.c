...
#undef CONDWAIT
#ifdef CONDWAIT
    while (empty_jobs(i) && !global_done && !live_node(root)) {
      //moet die i!= root niet ook in de whule conditie??????
      
      printf("M:%d Waiting (root@%d) jobs:%d\n", i, root->board, total_jobs);
